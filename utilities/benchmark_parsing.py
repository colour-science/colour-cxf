...
def generate_large_cxf_file(num_samples: int) -> bytes:
    ...
    print(f"Generated CxF file: {len(xml_bytes):,} bytes ({len(xml_bytes)/1024/1024:.1f} MB)")
    return xml_bytes


def benchmark_parsing_memory_usage(num_samples: int):
    """Benchmark parsing with memory profiling."""
    
    @profile
    def parse_cxf_with_validation(data: bytes):
        """Parse CxF with schema validation enabled."""
        return colour_cxf.read_cxf(data, validate_schema=True)
    
    @profile  
    def parse_cxf_without_validation(data: bytes):
...
def benchmark_parsing_performance(num_samples: int):
    ...
    print(f"   Objects parsed: {len(cxf_obj.resources.object_collection.object_value):,}")
    print(f"   Throughput: {len(cxf_obj.resources.object_collection.object_value)/parse_time_with_validation:.0f} objects/second")
    ...
    print(f"   Memory used: {memory_used:.1f} MB")
    print(f"   Objects parsed: {len(cxf_obj.resources.object_collection.object_value):,}")
    print(f"   Throughput: {len(cxf_obj.resources.object_collection.object_value)/parse_time_without_validation:.0f} objects/second")
    
    # Performance comparison
    speedup = parse_time_with_validation / parse_time_without_validation
    print(f"\n3. Performance comparison:")
    print(f"   Schema validation overhead: {(parse_time_with_validation - parse_time_without_validation):.2f} seconds")
    print(f"   Speedup without validation: {speedup:.2f}x")
    print(f"   Data throughput (with validation): {len(cxf_data)/1024/1024/parse_time_with_validation:.1f} MB/second")
    print(f"   Data throughput (without validation): {len(cxf_data)/1024/1024/parse_time_without_validation:.1f} MB/second")
